...
const BsFormData = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('Business mark saved successfully!');
      setFormData({
        subjectcode: 'busi10',
        regno: '',
        score: '',
      ...
      toast.error('Failed to save business mark. Please try again.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      toast.success('Business mark deleted successfully!');
      getFetchData(); // Refresh student data
    } catch (error) {
      toast.error('Failed to delete business mark. Please try again.');
      console.error(error);
    }
  };
            ...
            <table className="min-w-full">
              <thead className="bg-slate-800 text-white">
                <tr>
                  ...
                  <th className="border-r border-r whitespace-nowrap">BUSINESS SCORE</th>
              ...
              <tbody>
                {students.map((student, index) => (
                  <tr key={index} className="border py-3 border-slate-500">
                    ...
                    <td className="pr-2 text-center border border-slate-500">{student.marks.busi10}</td>
