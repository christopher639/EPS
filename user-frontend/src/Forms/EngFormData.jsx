...
const EngFormData = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success('English mark saved successfully!');
      setFormData({
        subjectcode: 'eng10',
        regno: '',
        score: '',
      ...
      toast.error('Failed to save English mark. Please try again.');
      console.error(error);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id) => {
    try {
      ...
      toast.success('English mark deleted successfully!');
      getFetchData(); // Refresh the student data
    } catch (error) {
      toast.error('Failed to delete English mark. Please try again.');
      console.error(error);
    }
  };
            ...
            <table className="min-w-full">
              <thead className="bg-slate-800 text-white">
                <tr>
                  ...
                  <th className="border-r whitespace-nowrap">ENG SCORE</th>
              ...
              <tbody>
                {students.map((student, index) => (
                  <tr key={index} className="border py-3 border-slate-500">
                    ...
                    <td className="pr-2 text-center border border-slate-500">{student.marks.eng10}</td>
