...
//4/12/2024
app.get("/api/joined-students-stream-mean/:stream", async (req, res) => {
  try {
    const { stream } = req.params; // Extract stream from URL parameters
    // Get student marks and averages for the given stream
    const studentMarks = await getJoinedStudentsMarks(stream);
    // Calculate the stream mean
    const totalAverage = studentMarks.reduce((sum, student) => sum + student.average, 0);
    const streamMean = studentMarks.length > 0 
      ? (totalAverage / studentMarks.length).toFixed(1) // Round to 1 decimal place
      : 0; // Default to 0 if no students
    res.status(200).json({
      stream,
      streamMean,
      students: studentMarks // Include student details for context
    });
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
...
const getJoinedStudentsMarks = async (stream) => {
  return await studentModel.aggregate([
    ...
    {
      // Step 3: Add total and average calculations
      $addFields: {
        total: { // Calculate total marks
          $sum: {
            $map: {
              input: "$marks",
              as: "mark",
        ...
        average: { // Calculate average marks
          $divide: [
            {
              $sum: {
                $map: {
            ...
            9 // Divide total by 9 for average
          ]
        }
      }
    },
    {
      // Step 4: Round the average to 1 decimal place
      $addFields: {
        average: { $round: ["$average", 1] }
      }
    },
    {
      // Step 5: Project required fields along with total and rounded average
      $project: {
        _id: 0, // Exclude the _id field
        name: 1, // Include the student name
        regno: 1, // Include the regno
        ...
        total: 1,    // Include total marks
        average: 1   // Include rounded average marks
      }
    },
    {
      // Step 6: Sort by average in descending order
      $sort: { average: -1 }
    }
  ]);
};
...
app.get("/api/streams-summary", async (req, res) => {
  try {
    const streamSummary = await studentModel.aggregate([
      {
        // Step 1: Lookup marks for each student
        $lookup: {
          from: "subjectmarks", // Collection to join
      ...
      {
        // Step 2: Add total and average calculations for each student
        $addFields: {
          total: { // Calculate total marks
            $sum: {
              $map: {
      ...
      {
        // Step 3: Round the average to 1 decimal place
        $addFields: {
          average: { $round: ["$average", 1] }
        }
      },
      {
        // Step 4: Group by stream to calculate class-level summaries
        $group: {
          _id: "$stream", // Group by stream
          totalStudents: { $sum: 1 }, // Count number of students in the stream
          totalAverageSum: { $sum: "$average" } // Sum of all students' averages in the stream
        }
      },
      {
        // Step 5: Calculate the stream mean
        $addFields: {
          streamMean: {
            $cond: {
              if: { $gt: ["$totalStudents", 0] }, // Avoid division by zero
              then: { $divide: ["$totalAverageSum", "$totalStudents"] }, // Calculate mean
              else: 0
            }
          }
        }
      ...
      {
        // Step 6: Round the stream mean to 1 decimal place
        $addFields: {
          streamMean: { $round: ["$streamMean", 1] }
        }
      },
      {
        ...
        $project: {
          _id: 0,
          stream: "$_id", // Include the stream name
          totalStudents: 1, // Include the total number of students
          streamMean: 1    // Include the rounded stream mean
        }
      },
      {
        // Step 6: Sort by average in descending order
    ...
    res.status(200).json(streamSummary);
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
});
...
// //12/12/2024
// // Aggregation endpoint to fetch filtered student marks
// app.get('/api/joined-students_marks/:stream', async (req, res) => {
//   try {
//       const { stream } = req.params;
//       const { searchQuery, year, term, examType } = req.query; // Get query params from the URL
//       // Constructing the aggregation pipeline
//       let pipeline = [
//           {
//               $match: { stream: { $eq: stream } }, // Match based on stream
//           },
//       ];
//       // Add filter for registration number (searchQuery)
//       if (searchQuery) {
//           pipeline.push({
//               $match: {
//                   regno: { $regex: searchQuery, $options: 'i' }, // Case-insensitive search
//               },
//           });
//       }
//       // Filter by year
...
//           });
//       }
//       // Filter by exam type (Opener, Mid-Term, etc.)
//       if (examType) {
//           pipeline.push({
//               $match: { examType: examType },
//           });
//       }
//       // Perform the aggregation with the dynamic pipeline
